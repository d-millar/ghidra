...
public class LldbModelTargetProcessLaunchWithOptionsConnectorImpl extends LldbModelTargetObjectImpl
	...
	protected Map<String, ParameterDescription<?>> computeParameters() {
		...
		ParameterDescription<Boolean> pF0 = ParameterDescription.create(Boolean.class, "Exec", false,
			false, "Exec", "exec when launching and turn the calling process into a new process");
		map.put("Exec", pF0);
		ParameterDescription<Boolean> pF1 = ParameterDescription.create(Boolean.class, "BreakOnLaunch", false,
			true, "Break on launch", "stop as soon as the process launches to allow the process to be debugged");
		map.put("BreakOnLaunch", pF1);
		ParameterDescription<Boolean> pF2 = ParameterDescription.create(Boolean.class, "BreakOnEntry", false,
			true, "Break on entry", "stop at the program entry point instead of auto-continuing");
		map.put("BreakOnEntry", pF2);
		ParameterDescription<Boolean> pF3 = ParameterDescription.create(Boolean.class, "ASLR", false,
			false, "Disable ASLR", "disable Address Space Layout Randomization (ASLR)");
		map.put("ASLR", pF3);
		ParameterDescription<Boolean> pF4 = ParameterDescription.create(Boolean.class, "STDIO", false,
			false, "Disable STDIO", "disable stdio for inferior process (e.g. for a GUI app)");
		map.put("STDIO", pF4);
		ParameterDescription<Boolean> pF5 = ParameterDescription.create(Boolean.class, "NewTTY", false,
			false, "New TTY", "launch the process in a new TTY if supported by the host");
		map.put("NewTTY", pF5);
		ParameterDescription<Boolean> pF6 = ParameterDescription.create(Boolean.class, "Shell", false,
			false, "Launch from shell", "launch the process inside a shell to get shell expansion");
		map.put("Shell", pF6);
		ParameterDescription<Boolean> pF7 = ParameterDescription.create(Boolean.class, "NewGroup", false,
			false, "New group", "launch the process in a separate process group if you are going to hand the process off (e.g. to debugserver)");
		map.put("NewGroup", pF7);
		ParameterDescription<Boolean> pF8 = ParameterDescription.create(Boolean.class, "ExitRace", false,
			false, "Suppress race on exit", "set this flag so lldb & the handee don’t race to set its exit status");
		map.put("ExitRace", pF8);
		ParameterDescription<Boolean> pF9 = ParameterDescription.create(Boolean.class, "Detach", false,
